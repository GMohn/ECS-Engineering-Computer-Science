      ...
      with open(solutionFileName, 'r') as solFil: #open the solution file
        ...
        if(out != sol):#output does not match solution
          ...
          for (i, (o,s)) in enumerate(zip(out,sol)):
            if o != s:
              break
          print('First mismatch occurs at word', i + 1)
          print('Your output for that word is', o,'but the correct word is', s)
...
if __name__ =='__main__':
  ...
  t = Tester('./leapyear.out', Tester.INPUT_STDIN, Tester.OUTPUT_STDOUT, 
              'Tests', 'Solutions', '.',)
  t.testAll()
