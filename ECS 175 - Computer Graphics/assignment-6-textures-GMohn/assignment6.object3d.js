    ...
    createVAO( gl, shader )
    {
       // throw '"ShadedObject3D.createVAO" is incomplete'
        ...
        if (location >= 0) {
            ...
            gl.enableVertexAttribArray(location);
                 offset =  offset +(3 * this.vertices.length/num_total_components) * Float32Array.BYTES_PER_ELEMENT;

           
            gl.vertexAttribPointer(location, this.num_components_vec3, gl.FLOAT, false, 0,  offset);
        }
        ...
        if (location >= 0 && this.material.hasTexture()) {
            ...
            gl.enableVertexAttribArray(location);
             offset=   offset +(3 * this.vertices.length/num_total_components) * Float32Array.BYTES_PER_ELEMENT;
            gl.vertexAttribPointer(location, this.num_components_vec2, gl.FLOAT, false, 0,  offset);
        }
