...
vec3 shadeAmbientLight(Material material, AmbientLight light) {
    ...
    return light.color * light.intensity * material.kA* texture(material.map_kD,o_a_texture_coord).rgb;
...
vec3 shadeDirectionalLight(Material material, DirectionalLight light, vec3 normal, vec3 eye, vec3 vertex_position) {
   ...
   vec3 result = vec3(0);
    ...
    result += LN * light.color * light.intensity * material.kD*texture(material.map_kD,o_a_texture_coord).rgb;

    // Specular
    vec3 R = reflect(L, N);
    result += pow( max(dot(R, V), 0.0), material.shininess) * light.color * light.intensity * material.kS *texture(material.map_nS,o_a_texture_coord).r;
...
void main() {
    ...
    normal = texture(u_material.map_norm, o_a_texture_coord).rgb;
