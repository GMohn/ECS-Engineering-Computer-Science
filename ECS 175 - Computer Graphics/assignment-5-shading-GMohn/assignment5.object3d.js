...
class Object3D
{
    ...
    createVAO( gl, shader )
    {
        ...
        let pos_shader = shader.getAttributeLocation('a_position');
        let pos_norm = shader.getAttributeLocation('a_normal');
        gl.enableVertexAttribArray(pos_shader);
        gl.vertexAttribPointer(pos_shader,this.num_components,gl.FLOAT,false,0,0);
                if(pos_norm >= 0){
            gl.enableVertexAttribArray(pos_norm);
            let offset = this.vertices.length/2 * 4;
            gl.vertexAttribPointer(pos_norm,this.num_components,gl.FLOAT,false, 0, offset);
        }
