...
class AmbientLight extends Light {
    ...
    update( ) {
     //   throw '"AmbientLight.update" not implemented'
        ...
        let light = `u_lights_ambient[${this.id}]`;
        this.target_shader.setUniform1f(light+'.intensity', this.intensity);
        this.target_shader.setUniform3f(light+'.color', this.color);
...
class DirectionalLight extends Light {
    ...
    update( ) {
        this.target_shader.use();
        let mat_model = mat4.create();
        let light_dir = vec3.fromValues(0,1,0);
        mat4.getRotation(mat_model,this.model_matrix);
        vec3.transformQuat(light_dir,light_dir,mat_model);
        let light = `u_lights_directional[${this.id}]`;
        this.target_shader.setUniform1f(light+'.intensity', this.intensity);
        this.target_shader.setUniform3f(light+'.color', this.color);
        this.target_shader.setUniform3f(light+'.direction', light_dir);
        this.target_shader.unuse();
    }
}
...
class PointLight extends Light {
    ...
    update( ) {
        ...
        // TODO: Pass the light properties to the shader
        this.target_shader.use();
        let vec_pos = vec3.create();
        mat4.getTranslation(vec_pos,this.model_matrix);
        let light = `u_lights_point[${this.id}]`;
        this.target_shader.setUniform1f(light+'.intensity', this.intensity);
        this.target_shader.setUniform3f(light+'.color', this.color);
        this.target_shader.setUniform3f(light+'.position', vec_pos);
        this.target_shader.unuse()
