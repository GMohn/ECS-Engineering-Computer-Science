...
import { WebGlApp, Point, Triangle, Square } from "../../assignment2.js"
...
class App
{
    ...
    update( )
    {
        ...
        // Drawing Shapes
        if ((this.app_state.getState('Draw Mode') == 'Discrete' && Input.isMouseClicked( 0 )) || 
            (this.app_state.getState('Draw Mode') == 'Continuous' && Input.isMouseDown( 0 ))) {
            ...
            switch(this.app_state.getState('Draw Shape')) {
                case 'Point':
                    this.impl.addPoint( this.gl, this.shader, position )
                break
                case 'Triangle':
                    let triangleSideLength = this.initial_width / 20 * Math.sqrt(2)
                    this.impl.addTriangle( this.gl, this.shader, position, triangleSideLength )
                break
                case 'Square':
                    let squareSideLength = (this.initial_width / 20) 
                    this.impl.addSquare( this.gl, this.shader, position, squareSideLength )
                break
            }
        }
